...
**管理者として再起動**すると、つながる場合は開発サーバー（Vite）の最新UIを使い、つながらないときだけ `dist` に切り替えます。  
「管理者で古い画面のまま」なら、トレイから一度終了して `run_app.vbs` → アプリ内の管理者再起動、または先に `npm run build` してから管理者起動してください。
