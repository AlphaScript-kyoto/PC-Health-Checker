...
- `run_app.vbs` / `run_app.bat` … 通常起動（`npm run dev`）
- `run_as_admin.bat` … 管理者として同じ開発起動（UAC あり）

※ アプリはトレイ常駐します。すでに動いていると新しい `npm run dev` はすぐ終了します。  
`run_app.bat` は起動前に古い Electron を終了します。手動ならトレイ右クリック → **終了** してから開き直してください。

※ `run_app.vbs` は文字化け防止のため英語のみです。コンソールが出ます。
...
| ディスク | CrystalDiskInfo 相当の識別情報。SMART 全項目は **詳細ウィンドウ** で確認 |
